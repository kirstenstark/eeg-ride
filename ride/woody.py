...
def woody(data, cfg, dur):
    ...
    for j in range(d3):
        data[:, :, j] = ride_detrend(data[:, :, j], [1, int(d1 * bd), int(d1 * (1 - bd)), d1])
    ...
    for iter in range(1):  # Woody's method is only iterated once
        ...
        for m in range(d3):
            ...
            if 'template' in cfg and cfg['template']['method'] == 'woody':
                template = temp1
                temp1 = template
            temp = data[:, :, m]
            ...
            for c in range(d2):
                temp11[:, c] = np.correlate(temp[:, c], temp1[:, c], mode='full')[d1 // 2 - dur : d1 // 2 + dur]
        ...
        temp5 = ride_detrend(temp5, [1, int(temp5.shape[0] * bd), int(temp5.shape[0] * (1 - bd)), temp5.shape[0]])
        ...
        latency = np.round(latency - np.median(latency))
